...
namespace project_kel8
{
    public partial class MasterKaryawan : Form
    {
        ...
        private void btnSaveKaryawan_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else 
                {
                    ...
                    {
                        MessageBox.Show("Phone number cannot more than 13 digits!", "Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                }
        ...
        {
            this.getGudangtblKaryawanTableAdapter.FillBy(this.dsEkspedisi.getGudangtblKaryawan, string.Empty);
        }
        ...
        {
            if (tbCariKaryawan.TextLength != 0)
            {
                string data = tbCariKaryawan.Text.ToString();
                cariData(data);
            }
            else
            {
                MessageBox.Show("please enter by Name", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                tbCariKaryawan.Focus(); // Fokuskan kembali ke TextBox agar pengguna dapat mengisi kembali
                //cariData("");
            }
        }
